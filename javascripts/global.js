...
$(document).ready(function() {
    ...
    $("form :input[type='textbox']:not(.date_normal, .date, .datetime, .datetime_normal):enabled:visible:first").select();
    ...
    $('a').live("click",function(){
        if ($(this).attr("href") && $(this).attr("href").toString().substr(0, 7) == 'http://' && $(this).attr("href").toString().indexOf('?', 0) >= 0 &&
...
jQuery.fn.extend({
    show : function(arguments) {
        ...
        if ($(this).get(0) && $(this).get(0).tagName == 'DIV' && $(this).attr('id') != undefined && $(this).attr('id').toString().substring(0, 3) == 'dlg') {
            ...
            $("#"+($(this).attr('id'))+" :input[type='textbox']:not(.date_normal, .date, .datetime, .datetime_normal):enabled:visible:first").focus();
        }
        return;
    }
});
    ...
    $.ajax({
        ...
        success: function(rXML){
            ...
            for (i = 0; i < des.length; i++) {
                ...
                var b = false;
                if (!document.getElementById('cmb_' + d[1]) && !document.getElementById('cmbc_' + d[1]) && !document.getElementById('cmbn_' + d[1]) && !document.getElementById('cmba_' + d[1])) b = true;
...
function processCheckList(form, action, name, no_confirm, custom_msg, new_tab) {
    var f = false;
...
function checkFileSize(obj) {
    ...
    if (Math.max(navigator.userAgent.toLowerCase().indexOf("msie"), 0)) {
        ...
        if (obj.files[0].fileSize > max_size) {
...
function showNextLookup(obj, key, where, offset) {
    ...
    $.ajax({
        type: "GET",
        url: "index.php",
        data: "rt=ajax/public_lookup&key=" + key + "&value="+valuesearch+"&where="+ strwhere +"&obj="+obj+"&offsets="+offset+"&where_string="+sys_lookup_where,
        success: function(rXML){
            if (!rXML) {
                alert("Failed to connect.");
            }
...
function getLookup() {
    ...
    $.ajax({
        type: "GET",
        url: "index.php",
        data: "rt=ajax/public_lookup&key=" + sys_lookup_key + "&value=" + encodeURIComponent(document.getElementById('txt_lookup_value').value)+"&where="+strwhere+"&obj="+sys_lookup_desc+"&offsets=0&where_string="+sys_lookup_where,
        success: function(rXML){
            if (!rXML) {
                alert("Failed to connect.");
            }
